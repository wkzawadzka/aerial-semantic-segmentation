...
class VGG16_UNet:
    def __init__(self, inputs, doBatchNorm: bool, doDropout: bool = True, learning_rate: float = 1e-3, dropout_rate:float = 0.3) -> None:
    ...
    def network(self):
        ...
        e6 = self.expand_block(c5, skip4, skip4.shape[1]) # incorporating skip features from last contracting step
        e7 = self.expand_block(e6, skip3, skip3.shape[1]) # incorporating skip features from 3rd contracting step
        e8 = self.expand_block(e7, skip2, skip2.shape[1]) # incorporating skip features from 2nd contracting step
        e9 = self.expand_block(e8, skip1, skip1.shape[1]) # incorporating skip features from 1st contracting step
