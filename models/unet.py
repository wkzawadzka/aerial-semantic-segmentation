...
class Binary_UNet:
    def __init__(self, inputs, conv_filters: int, doBatchNorm: bool = True, doDropout: bool = True, learning_rate: float = 1e-3, dropout_rate:float = 0.3) -> None:
        ...
        # optimizer: stochastic gradient descend with momentum (high momemtum of 0.99 according to org. U-Net paper)
        self.optimizer = SGD(learning_rate = learning_rate, momentum=0.99)
        # loss function: binary cross entropy: where 1 - presence of pine trees, 0 - absence
    ...
    def double_conv_block(self, inputs, n_filters: int, kernel_size = 3):
        ...
        # ---------- # conv 3x3, ReLU
        x = Conv2D(n_filters, kernel_size, padding='same', kernel_initializer='he_normal')(inputs)
    ...
    def contract_block(self, inputs, n_filters, kernel_size = 3, max_pooling = True):
        ...
        if max_pooling:
            next_layer = MaxPooling2D((2,2))(x) # a 2x2 max pooling operation with stride 2 for downsampling
    ...
    def expand_block(self, inputs, skip_connection, n_filters: int, kernel_size = 3):
        ...
        x = Conv2DTranspose(n_filters, (2,2), strides = (2, 2), padding = 'same')(inputs)
    ...
    def network(self):
        ...
        c5, skip5 = self.contract_block(c4, self.filters * 16, max_pooling=False) # max_pooling false, stop downsampling process

        # expansive path
        ''' /decoder/
        halfing number of filters in each expansive path step '''
        e6 = self.expand_block(c5, skip4, self.filters * 8) # incorporating skip features from last contracting step
        e7 = self.expand_block(e6, skip3, self.filters * 4) # incorporating skip features from 3rd contracting step
        e8 = self.expand_block(e7, skip2, self.filters * 2) # incorporating skip features from 2nd contracting step
        e9 = self.expand_block(e8, skip1, self.filters * 1) # incorporating skip features from 1st contracting step

        # outputs
        ''' using sigmoid activation layer for binary classification together with xavier weight initialization '''
        outputs = Conv2D(self.classes, 1, activation='sigmoid', kernel_initializer=GlorotUniform())(e9) #1x1 convolution
